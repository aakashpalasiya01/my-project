...
export const loginAction =
  (LoginData: any) => async (dispatch: AppDispatch) => {
...
export const logoutAction =
  (userId: string | number) => async (dispatch: AppDispatch) => {
    try {
      await API.get(
        `/wp-json/wp/v2/users/logout/?user_id=${userId}&${new Date()
          .getTime()
          .toString()}`
      );
...
export const emailVerify = async (activation_key: string) => {
  const response = await API.post("/wp-json/wp/v2/users/verify-account", {
    activation_key,
  });
  return response.data;
};
export const getPlanProductList = () => async (dispatch: AppDispatch) => {
  ...
  if (response.success)
    dispatch(authReducer.setPlanProducts(response.data.data));
  return response.data;
};
export const setSubscriptionPlan =
  (body: setCardDataBodyType) => async (dispatch: AppDispatch) => {
...
export const CancelSubscription =
  (body: { subscription_id: string | undefined }) =>
  async (dispatch: AppDispatch) => {
    const response = await API.post(
      "/wp-json/wp/v2/users/cancel-subscription/",
      body
    );
...
export const PausedSubscription = async (body: {
  subscription_id: string | undefined;
}) => {
  const response = await API.post(
    "/wp-json/wp/v2/users/pause-subscription/",
    body
  );
  return response;
};
export const ResumeSubscription = async (body: {
  subscription_id: string | undefined;
}) => {
  const response = await API.post(
    "/wp-json/wp/v2/users/subscription/resume",
    body
  );
  return response;
};
...
export const defaultStripCard = (body: AddStripCardBodyType) =>
  API.post("/wp-json/wp/v2/users/payment-method/set-default/", body);
